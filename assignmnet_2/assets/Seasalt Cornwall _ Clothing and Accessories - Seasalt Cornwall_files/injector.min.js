...
if(this.options.after){var wrapperCopy=$(this.options.after).clone();wrapperCopy.get(0).innerHTML='';this.options.wrapperHtml=wrapperCopy.get(0).outerHTML;}else{this.options.wrapperHtml='<li class="product product-item item last"></li>';}
...
return s4()+s4()+'-'+s4()+'-'+s4()+'-'+
s4()+'-'+s4()+s4()+s4();},insertWrapper:function(insert){$(insert).addClass('ambanners-injected');let numProducts=$(this.options.itemSelector).length;if(this.options.after){if(this.options.afterProductNum==-1){$(insert).insertBefore(this.options.after);}else{$(insert).insertAfter(jQuery('ol.products.list.items.product-items li')[this.options.afterProductNum-1]);}}else if(this.options.container){if(parseInt(this.options.afterProductNum)<=numProducts){$(this.options.container).append(insert);}}},top:function(){var top=0,self=this;if(this.options.wrappers.length>0){var tops={},max=0;$(this.options.wrappers).each(function(index,wrapper){var wrapperElement=$('#'+wrapper.id);if(wrapperElement.length){if(tops[wrapperElement.offset().top]){tops[wrapperElement.offset().top]++;}else{tops[wrapperElement.offset().top]=1;}}});this.tops=tops;Object.keys(tops).map(function(key,index){if(this.tops[key]>max){top=key;max=self.tops[key];}}.bind(this));}
return top;},resize:function(){$(this.options.element).hide();if(this.options.wrappers.length>0){$(this.options.wrappers).each(function(index,wrapper){if(!$('#'+wrapper.id).length){this.insertWrapper(wrapper);}}.bind(this));var top=this.top();$(this.options.wrappers).each(function(index,wrapper){if($(wrapper).offset().top!=top){$(wrapper).remove();}});this.showBanner();}},showBanner:function(){var width=0,insertWrapper,left=1000,screenWidth=window.innerWidth;$(this.options.wrappers).each(function(index,wrapper){if($('#'+wrapper.id).length){width+=$(wrapper).width();if($(wrapper).offset().left<left){left=$(wrapper).offset().left;}
insertWrapper=wrapper;}});if(insertWrapper){$(insertWrapper).append(this.options.element);if(width<screenWidth){$(this.options.element).css({'position':'absolute','width':width+'px'});}else{$(this.options.element).css({'position':'absolute','width':100+'%'});}
